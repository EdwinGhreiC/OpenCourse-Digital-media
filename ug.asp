...
<body>
	...
	<ul class="menu">
			...
			<% 	sql = "select * from cpt"
				...
				do while not rs.eof %>
			<li><a href="?cid=<%=rs("id")%>"><%= rs("chapter_title") %></a></li>
			<%  rs.movenext
				loop
				rs.close %>
	</ul>
	...
	<div class="container">
		<div class="chapter-info">
			<% 	chapter_id=trim(request.querystring("cid"))
				...
				if chapter_id<>"" then
					...
					if not rs.eof then
			%>
			<h1><%= rs("chapter_title") %></h1>
			<h2><%= rs("id") & "-" & section_id %></h2>
		...
		<div class="teaching-nav">
			<div class="section-choice">
				...
				<div class="dropdown">
				  ...
				  <ul class="dropdown-menu">
					<%  for i=1 to rs("sections_counts") %>
				    <li><a href="?cid=<%=chapter_id%>&sid=<%=i%>"><%= chapter_id & "-" & i %></a></li>
					<%	next %>
				  </ul>
				</div>
			...
			<div class="material-download">
				...
				<div class="btn-group">
				  ...
				  <ul class="dropdown-menu">
				    <li><a href="ug_pdf/<%=chapter_id%>-<%=section_id%>.pdf" target="_blank">课件</a></li>
				    <li><a href="#">素材</a></li>
				  </ul>
				</div>
			</div>
		...
		<div class="teaching-content">
			<%	if chapter_id<10 then chapter_id_txt="0" & chapter_id
				...
				video_path = "video/" & chapter_dir_name & "/" & chapter_id & "-" & section_id
